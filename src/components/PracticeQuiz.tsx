...
export function PracticeQuiz({ questions }: PracticeQuizProps) {
  ...
  return (
    <div className="space-y-6">
      {/* 顶部控制栏 */}
      <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-sm shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-colors"
            />
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/60">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <select
              ...
              className="bg-slate-950/80 border border-slate-800 rounded-lg px-2.5 py-1 text-slate-200 focus:outline-none focus:border-cyan-500/50"
            >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between text-xs text-slate-400 px-1">
          ...
          <span>点击选项可自测，点击右上角或答案区域可展开解析</span>
        </div>

        {filteredQuestions.length === 0 ? (
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center text-slate-400">
            <HelpCircle className="w-12 h-12 text-slate-600 mx-auto mb-3" />
            <p className="text-base font-semibold text-slate-300">没有找到匹配的题目</p>
          ...
          filteredQuestions.map((q, index) => {
            ...
            return (
              <div
                key={q.id}
                className="bg-slate-900/80 border border-slate-800/80 hover:border-slate-700/80 rounded-2xl p-5 md:p-6 backdrop-blur-sm shadow-lg transition-all"
              >
                {/* 题头元信息 */}
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded-md bg-slate-800 text-slate-300 text-xs font-mono font-bold">
                      #{index + 1}
                    </span>
                    ...
                    {q.tags?.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 rounded bg-slate-800/60 text-slate-400 text-xs">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-slate-400 hover:text-cyan-400 transition-colors shrink-0 ml-2"
                  >
                    {isRevealed ? (
                      <>
                      ...
                      <>
                        <Eye className="w-3.5 h-3.5 text-cyan-400" />
                        <span className="text-cyan-400">查看答案与解析</span>
                      </>
                    )}
                  </button>
                </div>
                  ...
                  <div className="flex items-center gap-2 mb-3 text-xs">
                    {isCorrect ? (
                      <span className="flex items-center gap-1 text-emerald-400 font-semibold">
                        <CheckCircle2 className="w-4 h-4" /> 自测回答正确！
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-rose-400 font-semibold">
                        <XCircle className="w-4 h-4" /> 您的选择: {currentSelection.join(', ')}，请查看下方标准答案
                      </span>
                    )}
                  </div>
                )}
                ...
                {/* 答案与深度解析区 */}
                {isRevealed && (
                  <div className="mt-4 pt-4 border-t border-slate-800/80 bg-slate-950/60 rounded-xl p-4 space-y-2.5 animate-fadeIn">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-400">官方标准答案:</span>
                      <span className="px-2.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-mono font-bold text-sm">
                        {q.answer}
                      </span>
                    </div>
